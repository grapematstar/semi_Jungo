	...
	<table class="pr_ListTable">
		...
		if(arr==null || arr.size()==0){
			...
			for(int i=0; i<arr.size(); i++){
				%>
				<tr>
					<td class="content_td"><%=arr.get(i).getRef() %></td>
					<td class="content_td"><%=arr.get(i).getType() %></td>
                    <td style="border-bottom: 1px solid gray;">
                    <a href="noticeContent.jsp?idx=<%=arr.get(i).getIdx()%>"><%=arr.get(i).getSubject() %></a></td>
                    <td class="content_td"><%=arr.get(i).getWriter() %></td>
                    <td class="content_td"><%=arr.get(i).getWritedate() %></td>                    
                    <td class="content_td"><%=arr.get(i).getReadnum() %></td>
				</tr>
				<%
			}
		}
